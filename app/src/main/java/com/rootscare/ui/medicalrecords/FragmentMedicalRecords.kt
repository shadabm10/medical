...
class FragmentMedicalRecords : BaseFragment<FragmentMedicalRecordsBinding, FragmentMedicalRecordsViewModel>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fragmentMedicalRecordsBinding?.btnAddMedicalRecord?.setOnClickListener(View.OnClickListener {
            (activity as HomeActivity).checkFragmentInBackstackAndOpen(FragmentAddMedicalRecord.newInstance())
        })

        if(isNetworkConnected){
            baseActivity?.showLoading()
            var getMedicalRecordListRequest= GetMedicalRecordListRequest()
            getMedicalRecordListRequest?.userId=fragmentMedicalRecordsViewModel?.appSharedPref?.userId
              ...
              //  Toast.makeText(activity, "Delete api will call", Toast.LENGTH_SHORT).show()
                ...
                CommonDialog.showDialog(context!!, object :
                    ...
                    override fun onConfirm() {
                        if(isNetworkConnected){
                            baseActivity?.showLoading()
                            var medicalFileDeleteRequest= MedicalFileDeleteRequest()
                            medicalFileDeleteRequest?.id=id
                            fragmentMedicalRecordsViewModel?.apideletepatientmedicalrecord(medicalFileDeleteRequest)
    ...
    override fun successMedicalRecordListResponse(medicalRecordListResponse: MedicalRecordListResponse?) {
        ...
        if (medicalRecordListResponse?.code.equals("200")){
            if (medicalRecordListResponse?.result!=null && medicalRecordListResponse?.result.size>0){
                fragmentMedicalRecordsBinding?.recyclerViewRootscareMedicalrecord?.visibility=View.VISIBLE
                ...
                fragmentMedicalRecordsBinding?.recyclerViewRootscareMedicalrecord?.visibility=View.GONE
