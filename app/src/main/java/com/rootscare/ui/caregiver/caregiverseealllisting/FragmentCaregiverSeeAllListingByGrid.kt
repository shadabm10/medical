...
import kotlinx.android.synthetic.main.fragment_see_all_caregiver_list_by_grid.*
import kotlinx.android.synthetic.main.fragment_see_all_nurses_list_by_grid.*

class FragmentCaregiverSeeAllListingByGrid: BaseFragment<com.rootscare.databinding.FragmentSeeAllCaregiverListByGridBinding, FragmentCaregiverSeeAllListingByGridViewModel>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
      ...
      //  setUpViewSeeAllNursesByGridlistingRecyclerview()
        ...
        fragmentSeeAllCaregiverListByGridBinding?.btnRootscareMoreCaregiver?.setOnClickListener(View.OnClickListener {
            (activity as HomeActivity).checkFragmentInBackstackAndOpen(
                FragmentCaregiverCategoryListing.newInstance())
        })
        fragmentSeeAllCaregiverListByGridBinding?.tvSeeAllCaregiveFilter?.setOnClickListener(View.OnClickListener {
        ...
        fragmentSeeAllCaregiverListByGridBinding?.edtCaregivergridSearchByName?.addTextChangedListener(object :
            TextWatcher {
...
//                if(s.toString().length>2){
                if(isNetworkConnected){
                    ...
                    nurseSearchByNameRequest?.searchContent=fragmentSeeAllCaregiverListByGridBinding?.edtCaregivergridSearchByName?.text?.toString()
                    fragmentCaregiverSeeAllListingByGridViewModel?.apisearchcaregiverGrid(nurseSearchByNameRequest)
                }
//                }
            }
        })
    ...
    override fun successAllCaregiverListingResponse(caregiverListingResponse: AllCaregiverListingResponse?) {
        ...
        if(caregiverListingResponse?.code.equals("200")){
            if(caregiverListingResponse?.result!=null && caregiverListingResponse?.result.size>0){
                fragmentSeeAllCaregiverListByGridBinding?.recyclerViewRootscareSeeallnursesByGrid?.visibility=View.VISIBLE
                ...
                fragmentSeeAllCaregiverListByGridBinding?.recyclerViewRootscareSeeallnursesByGrid?.visibility=View.GONE
                fragmentSeeAllCaregiverListByGridBinding?.tvNoDate?.visibility=View.VISIBLE
                fragmentSeeAllCaregiverListByGridBinding?.tvNoDate?.setText("No caregiver list found")
            }
        }else{
            fragmentSeeAllCaregiverListByGridBinding?.recyclerViewRootscareSeeallnursesByGrid?.visibility=View.GONE
            fragmentSeeAllCaregiverListByGridBinding?.tvNoDate?.visibility=View.VISIBLE
            fragmentSeeAllCaregiverListByGridBinding?.tvNoDate?.setText(caregiverListingResponse?.message)
...
//        trainerList: ArrayList<TrainerListItem?>?
        ...
        val contactListAdapter = AdapterCaregiverSeeAllListRecyclerview(allDoctorList,context!!)
...
//            fragmentSeeAllDoctorByGridBinding?.spinnerSpeciality?.adapter = customSpinnerAdapter
            fragmentSeeAllCaregiverListByGridBinding?.spinnerSpeciality?.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
