...
class FragmentAddMedicalRecord : BaseFragment<FragmentAddMedicalRecordsBinding, FragmentAddMedicalRecordViewModel>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fragmentAddMedicalRecordsBinding?.tvSelectTestDate?.setOnClickListener(View.OnClickListener {
...
//        checkPermissionAndProceedForImageCapture()
    ...
    private fun openFileMangerToPickUpFile() {
        ...
        startActivityForResult(Intent.createChooser(intent, "Select a file"),  AppConstants.MY_PERMISSIONS_REQUEST_READ_EXTERNAL_STORAGE)
    }
...
//            val layoutManager = GridLayoutManager(context, 2)
            ...
            fragmentAddMedicalRecordsBinding?.recyclerviewUploadFileList?.adapter = filesListingAdapter
...
//                    dataFileList.removeAt(position)
                    ...
                    filesListingAdapter?.notifyItemRangeChanged(position, filesListingAdapter?.data?.size!!)
                    imageListingReflectData()
                }
            }
        }
    ...
    private fun imageListingReflectData() {
        with(fragmentAddMedicalRecordsBinding!!) {
            if (dataFileList.size > 0) {
                fragmentAddMedicalRecordsBinding?.recyclerviewUploadFileList?.visibility = View.VISIBLE
            } else {
    ...
    private fun apiCallToInserMedicalRecordData(data: LinkedList<AddlabTestImageSelectionModel>?){
        ...
        if(isNetworkConnected){
            ...
            val user_id = RequestBody.create(MediaType.parse("multipart/form-data"), fragmentAddMedicalRecordViewModel?.appSharedPref?.userId)
            val date = RequestBody.create(MediaType.parse("multipart/form-data"), fragmentAddMedicalRecordsBinding?.tvSelectTestDate?.text?.toString())


            var titleString=""
            var p=0
            for (i in data?.indices!!) {
                var title:String= data[i].fileNameAsOriginal!!
                if (p==0){
                    titleString=title
                    p++
                }else{
            ...
            val title = RequestBody.create(MediaType.parse("multipart/form-data"), titleString)
            for (i in data?.indices!!) {
...
//            val body = MultipartBody.Part.createFormData(data[i].fileNameAsOriginal, data[i].rawFileName, requestFile!!)
            ...
            fragmentAddMedicalRecordViewModel?.apiinsertmedicalrecord(user_id,date,title,fileList)
        }else{
