...
class FilesListingAdapter(internal var activity: Activity) : RecyclerView.Adapter<FilesListingAdapter.ViewHolder>() {
...
//    var data: LinkedList<String> = LinkedList()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val singleItemDashboardListingBinding = DataBindingUtil.inflate<SingleItemFileListingBinding>(LayoutInflater.from(activity),
    ...
    inner class ViewHolder(var itemVie: SingleItemFileListingBinding) : RecyclerView.ViewHolder(itemVie.root) {
        ...
        fun onBind(myPosition: Int) {
            ...
            with(itemVie) {
                ...
                if (data[local_position].type != null) {
                    if (data[local_position].type?.trim()?.toLowerCase()?.contains("image")!! ||
                        data[local_position].fileName?.trim()?.toLowerCase()?.contains("jpg")!! ||
                        data[local_position].fileName?.trim()?.toLowerCase()?.contains("png")!!
                    ) {
                        if (data[local_position].filePath?.toUri() != null) {
                            Glide.with(activity).load(data[local_position].filePath.toString()).thumbnail(0.10f).timeout(1000*60).into(imgShow)
                        }
                    } else if (data[local_position].type?.trim()?.toLowerCase()?.contains("pdf")!! ||
                        data[local_position].fileName?.trim()?.toLowerCase()?.contains("pdf")!!
                    ) {
                        if (data[local_position].filePath?.toUri() != null) {
