...
class FragmentPaymentHistory : BaseFragment<FragmentPaymentHistoryBinding, FragmentPaymentHistoryViewModel>(), FragmentPaymentHistoryNavigator {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(isNetworkConnected){
            ...
            var patientPaymentHistoryRequest= PatientPaymentHistoryRequest()
            patientPaymentHistoryRequest?.userId=fragmentPaymentHistoryViewModel?.appSharedPref?.userId
    ...
    private fun setUpPaymentHistorylistingRecyclerview(paymentHistoryList: ArrayList<ResultItem?>?) {
        ...
        val contactListAdapter = AdapterPaymentHistoryRecyclerview(paymentHistoryList,context!!)
    ...
    override fun successPatientPaymentHistoryResponse(patientPaymentHistoryResponse: PatientPaymentHistoryResponse?) {
        ...
        if(patientPaymentHistoryResponse?.code.equals("200")){
            if(patientPaymentHistoryResponse?.result!=null && patientPaymentHistoryResponse?.result.size>0){
                ...
                fragmentPaymentHistoryBinding?.recyclerViewRootscarePaymenthistory?.visibility=View.GONE
                fragmentPaymentHistoryBinding?.tvNoDate?.visibility=View.VISIBLE
                fragmentPaymentHistoryBinding?.tvNoDate?.setText(patientPaymentHistoryResponse?.message)
            }
