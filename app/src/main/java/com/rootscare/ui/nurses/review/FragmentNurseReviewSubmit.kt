...
class FragmentNurseReviewSubmit : BaseFragment<FragmentSubmitFeedbackBinding, FragmentNurseReviewSubmitViewModel>(),
...
//                FragmentDoctorListingDetails.newInstance(doctorId))
    ...
    private fun submitReviewAndRatingApiCall(){
        if(isNetworkConnected){
            ...
            insertNurseReviewRequest?.rating=fragmentSubmitFeedbackBinding?.ratingBarteacherFeedback?.rating?.toString()
            insertNurseReviewRequest?.review=fragmentSubmitFeedbackBinding?.edtReattingComment?.text?.toString()?.trim()
