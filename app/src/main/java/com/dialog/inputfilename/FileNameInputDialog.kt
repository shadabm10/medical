...
class FileNameInputDialog (internal var activity: Activity, callbackAfterDateTimeSelect: CallbackAfterDateTimeSelect,
                           internal var booleanForResueInAnotherPage:Boolean = false, internal var tempString:String?=null) :
        ...
        with(layoutBinding) {
            if (v!!.id == tvSubmit.id) {
                if (!TextUtils.isEmpty(fileName.trim())){
                    ...
                    if (booleanForResueInAnotherPage){
                        edittextFilename.error = activity.resources.getString(R.string.pleaese_enter_some_description)
                    }else {
                        edittextFilename.error = activity.resources.getString(R.string.pleaese_create_name_for_this_file)
                    }
